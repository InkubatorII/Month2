# AUTOINCREMENT - автозаполнение айди +1
import sqlite3
with sqlite3.connect('game.db') as connection:
    ...
    # VALUES ('beka', 'DEADINSIDE'), ('beka', 'ZODIAC')''')
    ...
    FROM users
    LEFT JOIN games ON users.id = games.users_id
    ''')
    for row in cursor.fetchall():
        print(row)
    ...
    cursor.execute('''UPDATE users SET name="bekas"  WHERE id=1''')